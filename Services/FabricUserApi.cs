...
namespace OneLakeIntegrationDemo.Services {
  ...
  public class FabricUserApi {
    ...
    private static string AccessToken = AzureAdTokenManager.GetAccessToken(FabricPermissionScopes.TenantProvisioningPowerBi);
    ...
    public static FabricWorkspace CreateWorkspace(string WorkspaceName, string CapacityId = AppSettings.PremiumCapacityId, string Description = null) {
      ...
      //if (AppSettings.RunAsServicePrincipal) {
      //  // add admin user for demo purposes if created by service principal
      //  AddWorkspaceUser(workspace.id, AppSettings.AdminUser1Id, WorkspaceRole.Admin);
      //}
